...
- 当前开发版：普通模块改为 UniFi 风格的可视化资源页，包含完整分页、图标、实时指标、状态/类型分布、分类 Tabs、搜索表格、详情抽屉和对象级字段化操作；当前行可连续重复点击，所有可点击区域提供明确指针反馈；“全部官方端点”继续保留开发工作台
...
- 可独立展开/折叠的分组侧边栏：工作台、网络资源、策略与安全、基础设施与开发四个导航区互不影响
- 新增完整策略基线导入、导出和差异计算
...
Windows 与 macOS 的普通资源模块不再显示路径和 JSON：列表接口显示为分类 Tabs，并自动读取全部分页；官方响应转换为资源表格、实时状态指标和类型分布。没有状态字段的国家、DPI 等参考资源会明确显示“接口未返回状态”，不会被误算为正常。选择一行后，详情抽屉会按当前资源显示官方支持的详情、更新、动作与删除入口；官方只提供 GET 的资源则明确标记为只读。PUT 更新会先读取最新完整详情，并只把请求模板允许的字段预填到表单，写入前仍保留二次确认。只有“全部官方端点”入口保留端点搜索、路径参数、JSON 请求体和响应复制，供开发与故障排查使用。防火墙排序与 Hotspot 批量删除等集合级端点会明确提示官方必填查询参数。
