...
- 当前开发版：普通模块改为 UniFi 风格的可视化资源页，包含图标、实时指标、状态/类型分布、分类 Tabs、搜索表格、详情抽屉和字段化操作；“全部官方端点”继续保留开发工作台
...
Windows 与 macOS 的普通资源模块不再显示路径和 JSON：列表接口显示为分类 Tabs，官方响应转换为资源表格、实时状态指标和类型分布，选择一行后在详情抽屉中查看字段；创建、更新、动作和删除接口会转换为带字段说明的表单，并保留写操作二次确认。只有“全部官方端点”入口保留端点搜索、路径参数、JSON 请求体和响应复制，供开发与故障排查使用。防火墙排序与 Hotspot 批量删除等端点会明确提示官方必填查询参数。
